...
export default ['hs.map.service', 'Core', 'hs.utils.service', '$window', '$cookies', 'config', 'hs.layout.service', '$log', '$document', function (OlMap, Core, utils, $window, $cookies, config, layoutService, $log, $document) {
  const me = {
    /**
    ...
    */
    ...
    serializeStyle: function (s) {
      ...
      if (angular.isDefined(s.getImage()) && s.getImage() !== null) {
        ...
        const ima = {};
        if (angular.isDefined(style_img.getFill) && angular.isDefined(style_img.getFill()) && style_img.getFill() !== null) {
          ima.fill = style_img.getFill().getColor();
        }

        if (angular.isDefined(style_img.getStroke) && angular.isDefined(style_img.getStroke()) && style_img.getStroke() !== null) {
          ima.stroke = {
