...
angular.module('hs.draw', ['hs.map', 'hs.core', 'hs.utils', 'hs.geolocation'])
    ...
    */
    .controller('hs.draw.controller', ['$scope', 'hs.map.service', 'Core', 'hs.geolocation.service', '$http', 'hs.utils.service', '$timeout', 'hs.save-map.service', 'config',
        function ($scope, OlMap, Core, Geolocation, $http, utils, $timeout, saveMap, config) {
            var map = OlMap.map;
            var draw;
            var modify;
            var selector;
            var current_feature_collection = new Collection();
            var newObsId = 0;
