...
export default {
  ...
  controller: ['$scope', 'HsMapService', 'HsCore', 'HsGeolocationService', '$http', 'HsUtilsService', '$timeout', 'HsSaveMapService', 'HsConfig', 'HsDrawService', '$compile', 'HsLayoutService',
    function ($scope, OlMap, HsCore, Geolocation, $http, utils, $timeout, saveMap, config, drawService, $compile, layoutService) {
      ...
      const attrs_with_template_tags = ['category_id', 'dataset_id', 'description', 'name'];
      const attrs_not_editable = ['geometry', 'highlighted', 'attributes', 'sync_pending'];
      const attrs_dont_send = ['media_count', 'obs_vgi_id', 'time_stamp', 'unit_id', 'time_received', 'dop', 'alt', 'media'];
      ...
      function fillFeatureList() {
        ...
        $scope.features = [];
        angular.forEach(drawService.source.getFeatures(), (feature) => {
          if (angular.isDefined(feature.get('obs_vgi_id')) && feature.get('obs_vgi_id') >= newObsId) {
                          ...
                          dirReader.readEntries((results) => {
                            angular.forEach(response.data, (r) => {
                              ...
                              if (synced) {
                                ...
                                downloadFile($scope.senslog_url + '/observation/' + feature.get('obs_vgi_id') + '/media/' + r.media_id + '/thumbnail?user_name=' + config.user_name, (data) => {
      ...
      $scope.newPointFromGps = function () {
        ...
        function createPoint(pos) {
          const g_feature = pos ? new Point(pos.latlng) : new Point(transform([0, 0], 'EPSG:4326', OlMap.map.getView().getProjection()));
        ...
        function waitForFix() {
          ...
          const stopWaiting = $scope.$on('geolocation.updated', (event) => {
            ...
            if (Geolocation.last_location.geoposition.coords.accuracy < requiredPrecision) {
              const g_feature = new Point(Geolocation.last_location.latlng);
      ...
      $scope.addPhoto = function () {
        ...
        function cameraSuccess(imageData) {
          window.resolveLocalFileSystemURL(imageData, (fileEntry) => {
            ...
            const portalFolder = config.hostname.default.url.split('/').slice(-1)[0];
            window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, (fileSys) => {
              let obsId = angular.isDefined($scope.current_feature.ol_feature.get('obs_vgi_id')) ? $scope.current_feature.ol_feature.get('obs_vgi_id') : -1;
              let isNew = true;
              // var hasDir = false;
                  ...
                  if (isNew) {
                    ...
                    mediaDir.getDirectory('temp', {
                      ...
                      tempDir.getDirectory(String(obsId), {
                        ...
                        angular.forEach($scope.current_feature.media, (m) => {
                          if (m.media_id >= lastMediaId) {
                            lastMediaId = m.media_id + 1;
                          }
                        });

                        fileEntry.moveTo(obsDir, lastMediaId + '.jpg', (entry) => {
                    ...
                    mediaDir.getDirectory(String(obsId), {
                      ...
                      obsDir.getDirectory('temp', {
                        ...
                        getEntries();
                        fileEntry.moveTo(tempDir, lastMediaId + '.jpg', (entry) => {
                          $scope.current_feature.media.push({
                            url: entry.nativeURL,
                            image_type: $scope.image_type,
                            synced: false
      ...
      $scope.setCurrentFeature = function (feature, zoom_to_feature) {
        if ($scope.current_feature == feature) {
          ...
          $timeout(() => {
            ...
            const panelSpace = layoutService.contentWrapper.querySelector('.hs-panelplace');
     ...
     */
      ...
      function deleteVgiObservation(olf) {
        $http.delete($scope.senslog_url + '/observation/' + olf.get('obs_vgi_id') + '?user_name=' + config.user_name).then((response) => {
      ...
      $scope.sync = function () {
        angular.forEach($scope.features, (feature) => {
          ...
          const cord = transform(olf.getGeometry().getCoordinates(), OlMap.map.getView().getProjection(), 'EPSG:4326');
          ...
          if (config.uuid) {
            fd.append('uuid', config.uuid);
          }
          if (config.unit_id) {
            fd.append('unit_id', config.unit_id);
          }
                          ...
                          window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, (fileSys) => {
                            fileSys.root.getDirectory(portalFolder + '/media/', {
                              ...
                              mediaDir.getDirectory(String(obsId), {
                                ...
                                window.resolveLocalFileSystemURL(m.url, (fileEntry) => {
                                  fileEntry.moveTo(obsDir, m.media_id + '.jpg', (entry) => {
                                    m.synced = true;
