  ...
  function (
    ...
    const me = {
      ...
      updateMapSize: function () {
        ...
        if (
          layoutService.contentWrapper.getElementsByClassName('hs-panelspace')
            .length > 0
        ) {
          sidebarElem = layoutService.contentWrapper.querySelector(
            '.hs-panelspace'
          );
        }
        ...
        if (angular.isDefined(OlMap.map)) {
          ...
          $timeout(() => {
            map.offsetWidth < 767
              ? (layoutService.smallWidth = true)
              : (layoutService.smallWidth = false);
          }, 0);
        }
