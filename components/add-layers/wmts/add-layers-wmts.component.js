    ...
    function ($scope, OlMap, srv_caps, HsCore, $compile, layoutService, $log) {
      ...
      $scope.capabilitiesReceived = function (response) {
        try {
          ...
          for (let idx = 0; idx < caps.OperationsMetadata.GetTile.DCP.HTTP.Get.length; idx++) {
          ...
          const el = angular.element('<div hs.wmts.capabilities_error_directive></div>');
      ...
      $scope.setCurrentLayer = function (layer, index) {
        if ($scope.currentLayer == layer) {
          ...
          const wmtsLayerPanel = layoutService.contentWrapper.querySelector('.hs-wmts-layerpanel');
          const layerNode = layoutService.contentWrapper.querySelector('#wmtslayer-' + index);
          if (wmtsLayerPanel) {
      ...
      $scope.addLayer = function (layer) {
        ...
        const size = getWidth(projectionExtent) / $scope.layerTileMatrix.TileMatrix[0].TileWidth;
