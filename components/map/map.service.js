...
export default [
  ...
  function (config, $rootScope, utils, layoutService, $timeout, gettext) {
        ...
        me.map.on('wheel', (e) => {
          ...
          if (
            ...
            style="
              position: absolute;
              ${
                !layoutService.sidebarBottom() && layoutService.sidebarRight
                  ? 'right'
                  : null
              ...
              ${layoutService.sidebarBottom() ? 'bottom:' : null}: ${
              layoutService.panelSpaceHeight() + 5
            }px};"
            role="alert">
    ...
    this.controls = angular.isDefined(config.mapControls)
      ? config.mapControls
      : window.cordova
