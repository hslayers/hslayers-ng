 ...
 * @memberOf hs
 */
define(['angular', 'ol', 's4a', 'map', 'core'], (angular, ol, s4a) => {
  angular
    ...
    .controller('HsRoutingController', [
      ...
      function ($scope, OlMap, Core, layoutService) {
        ...
        const gjSrc = new ol.source.Vector();
        ...
        // layer
        const gjStyle = new ol.style.Style({
        ...
        // source and style
        const gjLyr = new ol.layer.Vector({
          source: gjSrc,
          style: gjStyle,
        });
          ...
          if ($scope.wayPoints.length < 3) {
            ...
            Routing.getOptimalRoute(nodeIds, start, finish).then((res) => {
              ...
              for (let i = 0; i < nodes.length - 1; i++) {
                Routing.getShortestRoute(nodes[i].id, nodes[i + 1].id).then(
                  (res) => {
          ...
          Routing.getNearestNode(lonlat[0], lonlat[1], 100).then((res) => {
            if (res.status === 'success') {
              gjSrc.addFeature(
                new ol.Feature({
                  geometry: new ol.geom.Point(
                    trans([res.data.lon, res.data.lat])
                  ),
                })
              );
              ...
              if (
                ...
                getShortestRoute(
                  $scope.wayPoints[0].id,
                  $scope.wayPoints[1].id
                );
              }
              if (!$scope.$$phase) {
                $scope.$digest();
