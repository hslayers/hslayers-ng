...
export default ['hs.map.service', 'config', 'Core', '$rootScope', 'hs.utils.service', 'hs.addLayersVector.service', function (OlMap, config, Core, $rootScope, utils, nonWmsService) {
  const me = {
    ...
    createVectorLayer: function (lyr_def) {
      ...
      switch (format) {
        case 'ol.format.KML':
          return nonWmsService.add('kml', decodeURIComponent(lyr_def.protocol.url), lyr_def.title || 'Layer', lyr_def.abstract, extractStyles, lyr_def.projection.toUpperCase(), options);
        case 'ol.format.GeoJSON':
          return nonWmsService.add('geojson', decodeURIComponent(lyr_def.protocol.url), lyr_def.title || 'Layer', lyr_def.abstract, extractStyles, lyr_def.projection.toUpperCase(), options);
        case 'hs.format.WFS':
          options.defOptions = lyr_def.defOptions;
          return nonWmsService.add('wfs', decodeURIComponent(lyr_def.protocol.url), lyr_def.title || 'Layer', lyr_def.abstract, extractStyles, lyr_def.projection.toUpperCase(), options);
