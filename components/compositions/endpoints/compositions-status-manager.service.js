  ...
  function (statusManagerService, config, utils, $q, $http) {
     ...
     */
      loadList(ds, params, bbox) {
        ...
        const textFilter = query && angular.isDefined(query.title) && query.title != '' ? '&q=' + encodeURIComponent('*' + query.title + '*') : '';
        ...
        me.canceler = $q.defer();
        $http.get(url, {timeout: me.canceler.promise}).then((response) => {
