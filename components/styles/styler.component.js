	...
	controller: [
		...
		function($scope, service, $sce, Core, $http, layoutService) {
			...
			$scope.getLayerSource = function(layer) {
				var src = [];
				if (angular.isDefined(layer.getSource().getSource)) src = layer.getSource().getSource();
			...
			$scope.save = function() {
				...
				if ($scope.imagetype != "none") {
					if (
						...
						(angular.isDefined($scope.iconfillcolor) || angular.isDefined($scope.iconlinecolor))
					) {
						var circle_json = {
			...
			function colorIcon() {
				...
				if (angular.isDefined($scope.iconlinecolor) && $scope.iconlinecolor != null)
					svgPath.style.stroke = $scope.iconlinecolor["background-color"];
				if (angular.isDefined($scope.iconlinewidth) && $scope.iconlinewidth != null)
