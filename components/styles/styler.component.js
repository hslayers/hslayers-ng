...
export default {
  ...
  controller: [
    ...
    function ($scope, service, $sce, HsCore, $http, layoutService) {
      ...
      $scope.save = function () {
        if (service.layer == null) {
          return;
        }
        const source = $scope.getLayerSource(service.layer);
        const style_json = {};
      ...
      function updateHasVectorFeatures() {
        ...
        const src = $scope.getLayerSource(service.layer);
        if (angular.isUndefined(service.layer) || service.layer == null) {
          return;
        }
