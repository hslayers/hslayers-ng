            ...
            var me = {
                ...
                */
                storeWaypoints: function () {
                    ...
                    var query = 'WITH <http://www.sdi4apps.eu/trips.rdf> DELETE {?t ?p ?s} INSERT {' + trip_url + ' ' + waypoints_url + ' "' + angular.toJson(waypoints).replace(/"/g, '\\"') + '"} WHERE {?t ?p ?s. FILTER(?t = ' + trip_url + '). }';
                    $http.post(
    ...
    */
    .controller('hs.trip_planner.controller', ['$scope', 'hs.map.service', 'Core', 'hs.trip_planner.service', 'config', 'hs.layout.service',
        function ($scope, OlMap, Core, service, config, layoutService) {
            ...
            var movable_features = new Collection();
            ...
            $scope.waypointAdded = function (wp) {
                var f = new Feature({
                    geometry: new Point(transform([wp.lon, wp.lat], 'EPSG:4326', OlMap.map.getView().getProjection().getCode())),
                    wp: wp
                });
                wp.feature = f;
                source.addFeature(f);
                ...
                f.on('change', function (e) {
                    ...
                    var new_cords = transform(f.getGeometry().getCoordinates(), OlMap.map.getView().getProjection().getCode(), 'EPSG:4326');
