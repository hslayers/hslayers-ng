export default {
  ...
  controller: ['$scope', 'hs.map.service', 'hs.legend.service', function ($scope, OlMap, service) {
    let map;
    ...
    angular.extend($scope, {
      ...
      addLayerToLegends: function (layer) {
        const descriptor = service.getLayerLegendDescriptor(layer);
        if (descriptor) {
      ...
      noLayerExists: function () {
        const visibleLayers = $scope.layerDescriptors
