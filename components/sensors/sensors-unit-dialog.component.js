    ...
    function ($scope, OlMap, sensorsService, layoutService, sensorService) {
      ...
      angular.extend($scope, {
        ...
        timeButtonClicked(interval) {
          ...
          sensorsService
            .getObservationHistory(sensorsService.unit, interval)
            .then((_) => sensorService.createChart(sensorsService.unit));
        },

        dialogStyle() {
          return {
            'visibility': sensorsService.unitDialogVisible
              ? 'visible'
              : 'hidden',
            'left': layoutService.sidebarBottom()
              ? '3px'
              : layoutService.panelSpaceWidth() + 10 + 'px',
            'width': layoutService.sidebarBottom()
              ? '100%'
              : 'calc(' + layoutService.widthWithoutPanelSpace() + ')',
