...
export default ['hs.utils.service', '$http', 'config', 'hs.map.service', 'hs.layout.service', '$rootScope', '$compile', '$timeout', '$interval', '$log', function (utils, $http, config, hsMap, layoutService, $rootScope, $compile, $timeout, $interval, $log) {
  const me = this;
  ...
  return angular.extend(me, {
    ...
    getObservationHistory(unit, interval) {
      return new Promise((resolve, reject) => {
        const url = utils.proxify(`${endpoint.url}/senslog-lite/rest/observation`);
    ...
    createChart(unit) {
      ...
      const chartData = {
        ...
        'width': layoutService.dialogAreaElement.querySelector('.hs-chartplace').offsetWidth - 40,
        'autosize': {
          'type': 'fit',
          'contains': 'padding'
        },
      ...
      try {
        vegaEmbed(layoutService.dialogAreaElement.querySelector('.hs-chartplace'), chartData);
      } catch (ex) {
