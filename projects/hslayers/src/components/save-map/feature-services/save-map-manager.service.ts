...
export class HsSaveMapManagerService {
  ...
  /**
   * Get current extent of the map, transform it into EPSG:4326 and save it into controller model
   * @param app - App identifier
   * @returns Extent coordinates
   */
  getCurrentExtent(app: string): BoundingBoxObject {
    const b = this.hsMapService
      .getMap(app)
      .getView()
  ...
  /**
   * Callback for saving composition with a new title
   * @param app - App identifier
   */
