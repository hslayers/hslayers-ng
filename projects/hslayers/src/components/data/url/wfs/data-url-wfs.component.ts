...
@Component({
  selector: 'hs-data-url-wfs',
  templateUrl: './data-wfs-layer.directive.html',
})
export class HsDataWfsComponent {
  url: string;
  error: any;
  addAll: boolean;
  isChecked: boolean;
  ...
  constructor(
    public HsDataWfsService: HsDataWfsService,
    ...
    this.hsEventBusService.owsCapabilitiesReceived.subscribe(
      async ({type, response}) => {
        if (type === 'WFS') {
          try {
            const bbox = await this.HsDataWfsService.parseCapabilities(
              response
            );
            if (this.layerToAdd) {
              for (const layer of this.HsDataWfsService.services) {
            ...
            if (e.status == 401) {
              this.HsDataWfsService.wfsCapabilitiesError.next(
                'Unauthorized access. You are not authorized to query data from this service'
              );
              return;
            }
            this.HsDataWfsService.wfsCapabilitiesError.next(e);
          }
        }
      }
    );
    ...
    this.HsDataWfsService.wfsCapabilitiesError.subscribe((e) => {
      this.hsLog.warn(e);
      this.url = null;
  ...
  private addLayer(layer, layerName: string, folder: string, srs): void {
    ...
    const new_layer = new VectorLayer({
      title: layerName,
      source: this.HsDataWfsService.createWfsSource(options),
      path: folder,
      renderOrder: null,
      removable: true,
    });
