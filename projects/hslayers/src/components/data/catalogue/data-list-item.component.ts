...
@Component({
  selector: 'hs-data-list-item',
  templateUrl: 'data-list-item.html',
})
export class HsDataListItemComponent {
  @Input() layer;
  ...
  async addLayerToMap(
    ...
    const availableTypes = await this.HsDataCatalogueService.addLayerToMap(
      ds,
      layer,
      this.selectedType
    );
  ...
  async showMetadata(
    ...
    this.hsDialogContainerService.create(HsDataMetadataDialogComponent, {
