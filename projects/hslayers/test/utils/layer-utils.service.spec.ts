...
describe('LayerUtilsService', () => {
  ...
  it('should buffer extent correctly in meters', () => {
    ...
    const bufferedExtent = bufferExtent(extent, getProjection('EPSG:3857'));
  ...
  it('should buffer extent correctly with degree units (WGS84)', () => {
    ...
    const bufferedExtent = bufferExtent(extent, getProjection('EPSG:4326'));
  ...
  it('should not buffer extent beyond WGS84 bounds', () => {
    ...
    const bufferedExtent = bufferExtent(
      extremeExtent,
  ...
  it('should buffer extent correctly in EPSG:5514', () => {
    ...
    const bufferedExtent = bufferExtent(extent, getProjection('EPSG:5514'));
  ...
  it('should return small but non-zero buffer for moderate distances (e.g., 3000 meters)', () => {
    const result = getPolynomialBufferFactor(3000);
  ...
  it('should return decreasing buffer for intermediate distances', () => {
    const result1 = getPolynomialBufferFactor(250);
    const result2 = getPolynomialBufferFactor(1000);
