export default ['$http', 'HsMapService', 'HsUtilsService', '$rootScope', function ($http, OlMap, utils, $rootScope) {
  ...
  this.currentProjectionSupported = function (srss) {
    ...
    angular.forEach(srss, (val) => {
      if (val.toUpperCase().indexOf(OlMap.map.getView().getProjection().getCode().toUpperCase().replace('EPSG:', 'EPSG::')) > -1) {
        found = true;
      }
    });
    return found;
  ...
  this.getUrl = function (url, use_proxy) {
    if (typeof use_proxy == 'undefined' || !use_proxy) {
      ...
      return '/cgi-bin/proxy4ows.cgi?OWSURL=' + encodeURIComponent(url) + '&owsService=WMS';
    }
  };
  return me;
}
