...
export default ['$http', 'hs.map.service', 'hs.utils.service', '$rootScope',
  function ($http, OlMap, utils, $rootScope) {
    ...
    this.requestGetCapabilities = function (service_url) {
      ...
      if (angular.isUndefined(params.request) && angular.isUndefined(params.REQUEST)) {
        params.request = 'GetCapabilities';
      } else
      if (angular.isDefined(params.request)) {
        params.request = 'GetCapabilities';
      } else
      if (angular.isDefined(params.REQUEST)) {
        params.REQUEST = 'GetCapabilities';
      }
      if (angular.isUndefined(params.service) && angular.isUndefined(params.SERVICE)) {
        params.service = 'wmts';
      }
      if (angular.isUndefined(params.version) && angular.isUndefined(params.VERSION)) {
    ...
    this.service2layers = function (capabilities_xml) {
      ...
      const query_formats = (caps.Capability.Request.GetFeatureInfo ? caps.Capability.Request.GetFeatureInfo.Format : []);
      const image_format = getPreferedFormat(image_formats, ['image/png; mode=8bit', 'image/png', 'image/gif', 'image/jpeg']);
      const query_format = getPreferedFormat(query_formats, ['application/vnd.esri.wmts_featureinfo_xml', 'application/vnd.ogc.gml', 'application/vnd.ogc.wmts_xml', 'text/plain', 'text/html']);
